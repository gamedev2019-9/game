						...
						FLOAT x, FLOAT y, FLOAT z, FLOAT r, FLOAT g, FLOAT b )
{
	ZeroMemory( &light, sizeof(D3DLIGHT7) );
	light.dltType        = ltType;
	light.dcvDiffuse.r   = r;
	light.dcvDiffuse.g   = g;
	light.dcvDiffuse.b   = b;
	light.dcvSpecular    = light.dcvDiffuse;
	light.dvPosition.x   = light.dvDirection.x = x;
	light.dvPosition.y   = light.dvDirection.y = y;
	light.dvPosition.z   = light.dvDirection.z = z;
...
//-----------------------------------------------------------------------------
// Name: Nk3DUtil_SetRotateCCWYMatrix()
// Desc: Create Rotation matrix about Y axis CounterClockWize
//-----------------------------------------------------------------------------
VOID Nk3DUtil_SetRotateCCWYMatrix( D3DMATRIX& mat, FLOAT fRads )
{
	D3DUtil_SetIdentityMatrix( mat );
	mat._11 =  cosf( fRads );
	mat._13 =  sinf( fRads );
...
//-----------------------------------------------------------------------------
// Name: Nk3dMath_VertexRotateByVertex()
// Desc: 한점을 기준으로 버틱스를 회전시키는 알고리즘
//-----------------------------------------------------------------------------
void Nk3dMath_VectorRotateYByVertex(D3DVECTOR &rotated, D3DVECTOR pivot, float fRads)
{
	D3DMATRIX mat;
	//D3DVECTOR *pSrcVec = (D3DVECTOR *)&rotated.x;
	//D3DVECTOR *pDestVec = (D3DVECTOR *)&rotated.x;
	...
	// get rotation matrix
	D3DUtil_SetRotateYMatrix(mat, fRads);

	// 로테이트
	D3DMath_VectorMatrixMultiply(rotated, rotated, mat);

...
class CD3DFramework7
{
	...
	// 픽킹 버퍼를 위한 내부 변수
	LPDIRECT3DDEVICE7    m_pd3dDevicePick;			// The D3D pick device
	LPDIRECTDRAWSURFACE7 m_pddsBackBufferPick;		// The backbuffer surface pick
	LPDIRECTDRAWSURFACE7 m_pddsZBufferPick;				// The zbuffer surface
	...
	HRESULT CreateWindowedBuffers( DDSURFACEDESC2* );
...
public:
	...
	LPDIRECTDRAWSURFACE7 GetZBuffer()
	{
		return m_pddsZBuffer;
	}
	DWORD                GetRenderWidth()
	{
		return m_dwRenderWidth;    // Dimensions of the render target
	}
	DWORD                GetRenderHeight()
	{
		return m_dwRenderHeight;    // Dimensions of the render target
	}

	// 픽용 오브젝트 액세스 함수
	LPDIRECT3DDEVICE7    GetD3DDevicePick()
	{
		return m_pd3dDevicePick;
	}
	VOID				 SetPickView(BOOL bView)
	{
		m_bPickView = bView;
	}
	LPDIRECTDRAWSURFACE7 GetBackBufferPick()
	{
		return m_pddsBackBufferPick;
	}
	LPDIRECTDRAWSURFACE7 GetZBufferPick()
	{
		return m_pddsZBufferPick;
	}

	DWORD GetDeviceMemType()
	{
		return m_dwDeviceMemType;
	};	// 정원이가 집어 넣은것.

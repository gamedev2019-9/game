...
BOOL CLand::Init(char *strFilePath, IndexedTexture *pIndexedTexture, int nMapNum)
{
	...
	{
		MessageBox(NULL, "속성맵을 읽지 못했습니다", "경고", MB_OK);
	...
	if ( ((gMapWidth*5) % 8) != 0)
	{
		MessageBox(NULL, "맵의 가로길이는 8의 정수배가 되어야 합니다", "WARning", MB_OK);
	}
	...
	{
		MessageBox(NULL, "속성 데이타를 못 읽었습니다", "Warning", MB_OK);
		return FALSE;
	}
	...
	for (i = 0; i < m_nVisibleCount; i++)
	{
		...
		if ( ( (float) m_pHeightMap[h*gHFWidth+w]*gHeightScale) < m_fWaterAltitude )
		{
			if (g_bDungeon)
				...
				RenderWaterRect(w, h, alpha, lpTexture, lpTexture2);
...
void CLand::OnSaveHeightMap()
{
	...
	if (m_pHeightMap != NULL && bSuccess)
	{
		...
		if (count != gHFWidth*gHFHeight)
			MessageBox(NULL, "OnSaveHeightMap - 맵 크기로 저장되지 않음.", "경고", MB_OK);
		fclose(fp);
	}
}

...
HRESULT TextureContainer::LoadBitmapFile( TCHAR* strPathname, bool bDummy )
{
	...
	if (pfread((LPSTR)&bmfHeader, sizeof(bmfHeader), 1, fp) <= 0)
	{
		pfclose(fp);
		return E_FAIL;
	}
...
HRESULT TextureContainer::LoadTargaFile( TCHAR* strPathname, bool bDummy )
{
	...
	{	
		char dummy[10];
		pfread( dummy, 8, 1, file );

		offsetDummy = 8;
	}

	TargaHeader tga;
	pfread(&tga, sizeof(TargaHeader), 1, file );
...
HRESULT TextureContainer::LoadNTXFile( TCHAR* strPathname )
{
	...
	pfread( &m_dwWidth, sizeof(DWORD), 1, fp );
	pfread( &m_dwHeight, sizeof(DWORD), 1, fp );
	pfread( &m_dwBPP, sizeof(DWORD), 1, fp );
	pfread( &m_dwMipmapCount, sizeof(DWORD), 1, fp );
	...
	for(DWORD i = 0; i < m_dwMipmapCount; ++i)
	{
		...
		if( pfread( m_ppNTXData[i], sizeof(BYTE), 3 * dwWidth * dwHeight, fp ) < 3 * dwWidth * dwHeight )
		{
...
HRESULT TileTextures::LoadWxyFile( TCHAR* strPathname )
{
	...
	pfseek(fp, bmfHeader.bfOffBits+6, SEEK_SET);
	int  bytes = (bmInfo.bmiHeader.biWidth * bmInfo.bmiHeader.biBitCount + 15) / 16 * 2;
	bytes *= bmInfo.bmiHeader.biHeight;
	pfread(lpBits, bytes, 1, fp);

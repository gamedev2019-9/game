...
HRESULT D3DEnum_EnumerateDevices( HRESULT (*AppConfirmFn)(DDCAPS*, D3DDEVICEDESC7*) )
{
	...
	g_dwNumDevicesEnumerated = 0; // 다시 시작 버그 수정. (050805 by 원석)
	g_dwNumDevices = 0;           // 이 두개 변수를 초기화 안해줘서 문제가 발생했었다.
...
VOID D3DEnum_FreeResources()
{
	...
	{
		SAFE_DELETE_ARRAY( g_pDeviceList[i].pddsdModes );
	}
}
